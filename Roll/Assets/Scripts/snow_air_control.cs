...
using UnityEngine;
using Uniduino;
public class snow_air_control : MonoBehaviour {
	...
	void Start( )
	{
		...
		arduino.digitalWrite(pinRelay1,Arduino.LOW); // cold air flowing 
		arduino.digitalWrite(pinRelay2,Arduino.HIGH); // cold air flowing
	}

	void Update()
	{

		snow_airCheck (); // call to function to check the air status 
	}
	...
	void snow_airCheck ()
	{
		...
		for(int i = 0;i<fires.Length;i++) // looping the array 
		{
			...
			{
				arduino.digitalWrite(pinRelay1,Arduino.HIGH); // hot air is blowing  
